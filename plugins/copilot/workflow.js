export const meta = {
  name: '4x-loop-copilot',
  description: 'Designer-Coder-Reviewer-Tester loop (Copilot runner - experimental)',
  phases: [
    { title: 'Design', detail: 'Analyze requirements, produce spec + acceptance criteria', model: 'copilot' },
    { title: 'Code', detail: 'Implement per task-brief', model: 'copilot' },
    { title: 'Review', detail: 'Checklist + adversarial review', model: 'copilot' },
    { title: 'Test', detail: 'Validate against acceptance criteria', model: 'copilot' },
    { title: 'Amend', detail: 'Designer adjusts spec (on escalation)', model: 'copilot' },
    { title: 'Accept', detail: 'Final acceptance + commit plan', model: 'copilot' }
  ]
}

// This workflow mirrors plugins/claude-code/workflow.js but targets the Copilot CLI runner.
// Copilot runner is experimental: prompts are passed via a file (promptFile substitution).
...
// Model config (for Copilot plugin, models map to role labels; Copilot runner is single-model)
...
phase('Design')
log(`${featureId}: Starting design phase (Copilot)`)

const featureYaml = `${dotDir}/features/${featureId}.yaml`

const designResult = await agent(`You are the Designer (Copilot runner).
...
`, {
  ...
  // Runner-specific hint: use promptFile substitution when invoking copilot-cli; runtime should write prompt to temp file
  runnerHint: 'promptFile'
})
...
while (round < maxRounds) {
  ...
  // Code
  phase('Code')
  log(`${featureId}: Round ${round} — Coder (Copilot)`)

  const prevTestReport = round > 1 ? `${featureDir}/rounds/round-${round - 1}/test-report.md` : null

  const codeResult = await agent(`You are the Coder for round ${round} (Copilot).
...
`, {
  ...
  const review = await agent(`You are the Reviewer for round ${round} (Copilot).
...
`, {
  ...
  const testResult = await agent(`You are the Tester for round ${round} (Copilot).
