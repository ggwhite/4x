...
| `/api/logs/{id}/{file}` | GET | Get a specific log file (codex JSONL round logs are rendered to readable text on read; see below) |
...
The codex runner writes its round log as line-delimited JSON (`codex exec --json`). Both display paths render this to readable text on the fly — the original JSONL file is never modified. A log is recognized as codex by content (the first non-empty line parses as a codex event: `thread.` / `turn.` / `item.` type), not by filename. For each codex line, `agent_message` items become their `text`, `error` items and `turn.failed` become a short warning, other codex events (`thread.started`, `turn.completed`, reasoning, etc.) are dropped, and any non-codex line passes through unchanged — so claude stream-json logs are unaffected and byte-identical. The REST endpoint returns the rendered text plus an `X-Log-Raw-Size` header carrying the original file's byte length; the client uses that value (not the rendered length) as the SSE resume offset, so live-tail continues from the true file end without re-sending or leaking raw JSON. The SSE stream buffers codex output by line, holding a partial line in a carry until its newline arrives so a JSON event split across ticks is never emitted half-parsed; its raw-byte offset still advances by bytes read, decoupled from the emitted text length.
