...
### Costo total autoritativo

`Workspace.TotalCost` sigue el mismo patrón de "escanear todo el rastro de auditoría" que `WorktreePath` arriba, pero suma en lugar de tomar el último match: lee cada evento `run-end` en `events.jsonl` y suma su `cost_usd`. Esta es la única fuente autoritativa del costo total de un feature, compartida tanto por la CLI (el resumen final de `4x run`, sembrado en `orchestrator.NewRunner` para que una ejecución reanudada incluya el costo del proceso anterior) como por el dashboard (el campo `totalCostUSD` de `/api/messages/{id}`). Si `events.jsonl` no existe (feature nuevo, sin ejecuciones aún) retorna `(0, nil)`; una línea con formato inválido se omite en lugar de abortar todo el escaneo, para que un dato corrupto no oculte el costo del resto de las ejecuciones.
