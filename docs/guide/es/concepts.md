...
| **Acceptor** | `accepting` | Agrega todos los hallazgos de cada ronda en `final-report.md` y `commit-plan.md` para revisión humana |

El Acceptor usa su propia configuración de modelo dedicada (`roles.acceptor.model`) — distinta del Designer. Lee TODOS los artefactos de todas las rondas antes de producir el resumen final.
...
└── {feature-id}/
    ...
    ├── commit-plan.md               # Cómo dividir los cambios en commits
    ├── logs/
