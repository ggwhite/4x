...
**Audit counters**: every command that actually runs also records an `audit` object (`outputLines`, `goTestsRun`, `passLinesTopLevel`, `passLinesIndented`), computed from the full combined output before the summary is truncated. Blocked and skipped commands have no `audit`. All-zero counters mean the command passed without producing anything auditable — for example a `grep -q` that swallowed an upstream failure — which is exactly the kind of silently degraded verification a boolean pass/fail cannot show. Separately, the verify commands themselves are statically prechecked before the feature leaves `designing`, so unrunnable commands (unknown executable, missing path, exit code discarded by `;` or a pipe, unanchored `--- PASS:` grep) are rejected up front rather than passing vacuously; append `4x-lint:allow` to a command to skip that precheck.
...
In a multi-repo workspace, any root-level `shared_paths` the feature declared are merged back into the main workspace before the worktree is removed, and committed to the main workspace root repo as `feat(<feature-id>): <name>` (path-scoped, separate from the `chore(...)` pipeline-state commit above). The merged paths are printed as `shared-path merged: <path>`, and anything that could not be committed or propagated as `shared-path WARNING: <note>`. With `--json`, `4x done`, `4x force-done` and `4x merge` report them as `sharedPaths` (string array) and `sharedPathsNotes` (string array); both are omitted when empty. `4x done` and `4x force-done` additionally report `error` (string) in `--json` when the merge fails, so a consumer can tell a genuine `pending-review` apart from a merge that was blocked — for example by a `shared_paths` drift abort. `4x merge` does not need it: it exits non-zero and surfaces the reason through the standard error path.
