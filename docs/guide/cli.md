...
The loop drives: init → designing → design-reviewing → coding → reviewing → testing → deep-reviewing → fixing → accepting → pending-review. On review failure, code gets another pass. On test failure, the loop re-enters coding. The fixing phase runs after deep-reviewing PASS to address WARNING/INFO issues from the deep review report; it only activates when the profile enables it.
