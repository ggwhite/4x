...
`Workspace.TotalCost`는 위의 `WorktreePath`와 동일하게 "감사 추적 전체를 스캔"하는 패턴을 따르지만, 마지막 항목을 취하는 대신 합산한다는 점이 다릅니다: `events.jsonl`의 모든 `run-end` 이벤트를 읽어 각각의 `cost_usd`를 더합니다. 이는 기능의 총 비용에 대한 단일 권위 있는 소스이며, CLI(`4x run`의 요약, `orchestrator.NewRunner`에 seed되어 중단 후 재개된 run도 이전 프로세스의 비용을 이어받음)와 대시보드(`/api/messages/{id}`의 `totalCostUSD` 필드) 모두 이 함수를 공유합니다. `events.jsonl`이 존재하지 않으면(아직 실행된 적 없는 새 기능) `(0, nil)`을 반환합니다. 잘못된 형식의 줄은 전체 스캔을 중단시키지 않고 건너뛰므로, 한 줄의 손상된 데이터가 다른 모든 실행의 비용을 가리지 않습니다.
...
`Workspace.DiscoverScreenshots`는 각 라운드의 `verify.json`을 읽어 Tester가 기록한 스크린샷 증거를 수집합니다. 단일 라운드의 `verify.json`이 손상될 수 있습니다 — 예를 들어 캡처된 subprocess 출력에 이스케이프되지 않은 원시 ANSI 이스케이프 코드가 섞여 들어가는 경우 — 이는 JSON 파싱 실패로 이어집니다. 이 오류를 치명적 오류로 전파시켜 전체 탐색 호출(그리고 결과적으로 기능 전체의 `4x status`/`4x check`)을 실패시키는 대신, 파싱 실패는 best-effort로 처리됩니다: 해당 라운드는 verify.json에서 얻는 스크린샷을 전혀 제공하지 않지만 라운드 번호는 계속 추적되며, 다른 모든 라운드의 증거와 디렉터리 스캔 폴백은 정상적으로 처리됩니다.
