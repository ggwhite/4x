...
| `output_format` | `"stream-json"`으로 설정하면 runner stdout을 읽기 쉬운 `.log`와 raw `.stream.jsonl`로 기록합니다. |
| `tty` | 출력 캡처를 위한 PTY 사용. `output_format`이 `"stream-json"`이면 PTY를 사용하지 않습니다. |
