...
**Selection** (high→low): `--profile` wins; then the feature YAML's `profile` field (a named profile that must exist in `profiles` or the built-ins, else an error); then `default_profile`; then priority-based auto-select when a `profiles` section exists (highest priority → `full`, then `normal`, then `quick`), else `full`. On an interactive terminal 4x prompts with a numbered menu when none of the higher-priority sources resolve it.

**Profile advisor.** When no profile is specified, `4x new` and `4x run` print a deterministic profile *suggestion* (no LLM) scored from the feature's structural signals — subtask count, repo breadth, description rune length, `priority`, and refactor-keyword hits — with a per-signal reason breakdown. It is strictly advisory: non-interactive resolution is unchanged, and the interactive menu merely preselects the suggested profile as its cursor default (still requiring confirmation). It stays silent when a profile is already set, on `--json`, when disabled, or when the computed profile name does not exist in `profiles` ∪ built-ins. Weights and thresholds live under the `profile_advisor` settings key (see [Configuration → Profiles](configuration.md#profiles)).

**Per-phase runner/model precedence** (high→low): this-run-only `--phase-override <phase>:<runner>:<model>` (also sent by the dashboard run dialog; never persisted) > manual `--runner` > the feature YAML's `phase_overrides.<phase>.{runner,model}` > the profile's per-phase `runner`/`model` > `roles.<role>.runner` > `default_runner` / the role's configured model (`roles.<role>.model` → runner model → default tier). The dashboard run dialog previews the merged result via `POST /api/run/preview`, which shares the same `ResolvePipeline` resolution path the run loop uses, so the preview matches what actually executes.

Model tiers are named with brand-neutral canonical values — `strong` (judgement-heavy / deep review) and `fast` (routine, high-volume) — which `4x init` scaffolds and the built-in runner registry keys on. The legacy Claude-centric names `opus` / `sonnet` remain valid as backward-compatible aliases (`strong`↔`opus`, `fast`↔`sonnet`), resolved by a bidirectional fallback in tier lookup where the exact tier name takes priority over its alias; existing settings.json files and `--phase-override :opus` calls are unaffected.
...
For attribution *across* features and *within* a feature (per role, per round), the `4x cost` command reads the per-invocation `logs/*.stream.jsonl` files as its primary source — each carries a `total_cost_usd` in its `result` event, and its filename (`round-<N>-<role>.stream.jsonl`) encodes the round and role. `events.jsonl` `run-end` cost is used only as a fallback for features predating stream logs. Stream logs missing `total_cost_usd` are counted as skipped rather than aborting. See `docs/guide/cli.md` (`4x cost`) for the flags and output shape.

The codex runner is billed differently: it runs on a ChatGPT subscription with no USD metering, so 4x observes its *quota percentage* instead of a dollar amount. Its preset passes `--json`, which turns codex's stdout into a JSONL event stream; the first `thread.started` event's `thread_id` is the session id that names the matching rollout file (`<CODEX_HOME>/sessions/**/rollout-*-<id>.jsonl`). After each codex invocation the loop parses that rollout's last non-null `token_count` event for the 5-hour and weekly window `used_percent`, storing them on the `run-end` event's optional `codex` object (a `CodexUsage` struct — nil for non-codex runners, so a legitimate 0% is distinguishable from "not observed"). Any parse failure (no session id, missing rollout, changed format) is a warn-and-skip: the `run-end` is still written without the `codex` field, never blocking the round. `4x status` and `4x cost --feature` surface this quota per round; codex never enters the USD totals.
...
In a multi-repo workspace, declared `shared_paths` are copied into the worktree at setup and merged back into the main workspace before the worktree is removed by `4x done` / `4x merge` / `4x force-done`, then committed to the main workspace root repo as `feat(<feature-id>): <name>`. Anything that cannot be committed or propagated (file deleted on the worktree side, path excluded by `.gitignore`, workspace root not a git repo) is reported explicitly rather than skipped silently. A declared value must be a root-level *file*: a value that resolves to a directory in the workspace root is rejected, and so are `go.work` / `go.work.sum` (4x rewrites a trimmed `go.work` inside every worktree, so merging it back would truncate the real one). Both rules apply in monorepo and multi-repo workspaces alike — they constrain the shape of the declaration, not the merge-back path.

While the feature runs, the main workspace copies of the declared paths must stay identical to the snapshot 4x took when it first saw the declaration (`.4x/run/<feature-id>/shared-paths-baseline.json`). That snapshot is taken by the first `4x check` that reads the declaration, not at worktree setup — the run loop only forces a `4x check` after coding, so the sampling point can be as late as the end of the coding phase, and the protection window starts there. If someone changes them in the main workspace meanwhile, `4x check` fails and `4x done` aborts before touching any repo. To clear that state, first merge those main-workspace changes into the same-named files inside the worktree (or merge the two versions by hand) and confirm the worktree copy now contains the other side's content; only then delete `.4x/run/<feature-id>/shared-paths-baseline.json` to re-baseline and re-run. Do not clear it by changing the main workspace back to its previous content — merge-back always overwrites the main workspace with the worktree version, so reverting would simply erase whatever a parallel feature had just landed there.
...
For deterministic, exit-code-driven AC verification the Designer can instead declare `ac_checks` in `test-strategy.yaml` (`TestStrategy.ACChecks` in `internal/protocol/verify.go`), binding each execution-type AC to one or more commands. `4x verify` runs them and records the real exit codes into `ac_results[].checks` (`ACEvidence.Checks`), and the guard recomputes each AC's `passed` from those exit codes as the authoritative verdict — overriding any hand-written `passed`. Once `ac_checks` is declared, the guard also enforces that every execution-type AC binds at least one check, and a fake-verification linter rejects checks that only grep source, `echo`, `true`, or `:`. If `project.verify_command_allowlist` is non-empty, these AC check commands go through the same prefix/substitution guard as `verify_commands` and blocked checks are recorded with exit code `126` instead of being executed. Before the feature leaves `designing`, every command in `verify_commands`, `verify_groups` and `ac_checks` also goes through a purely static precheck: it never runs the command and never looks at an exit code, it only rejects commands that cannot work as written — an executable missing from PATH, a path or `cd` target that does not exist, an exit code discarded by `;` or by a pipe into a filter tool, or a `--- PASS:` grep pattern that is not anchored to the line start with a trailing space. A rejected command sends the feature back to the Designer with the offending command, the rule ID and the source in `guard-feedback.json`; appending the literal token `4x-lint:allow` skips the precheck for that command. See `docs/architecture/protocol.md` for the full format.
