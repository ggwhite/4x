...
`Workspace.TotalCost` は上記の `WorktreePath` と同じ「監査証跡全体をスキャンする」パターンに従いますが、最後の一致を取るのではなく合計する点が異なります：`events.jsonl` 内のすべての `run-end` イベントを読み取り、それぞれの `cost_usd` を加算します。これは Feature の総コストの唯一の権威あるソースであり、CLI（`4x run` の最終サマリー。`orchestrator.NewRunner` に seed されるため、中断後に再開した run でも前のプロセスのコストを引き継げる）と dashboard（`/api/messages/{id}` の `totalCostUSD` フィールド）の両方がこの関数を共用します。`events.jsonl` が存在しない場合（まだ一度も実行されていない新規 Feature）は `(0, nil)` を返します。不正な形式の行はスキャン全体を中断せずスキップされるため、1 行の不正データが他のすべての実行コストを隠してしまうことはありません。

### スクリーンショット検索の耐障害性

`Workspace.DiscoverScreenshots` は各ラウンドの `verify.json` を読み取り、Tester が記録したスクリーンショットの証跡を収集します。単一ラウンドの `verify.json` が破損していることがあります——例えば、キャプチャした subprocess の出力に未エスケープの生の ANSI エスケープコードが混入している場合——これは JSON パースの失敗を引き起こします。このエラーを致命的エラーとして伝播させ、探索呼び出し全体（ひいては Feature 全体の `4x status`/`4x check`）を失敗させる代わりに、パース失敗は best-effort として扱われます：そのラウンドは verify.json 由来のスクリーンショットを一切提供しませんが、そのラウンド番号は引き続き記録され、他のすべてのラウンドの証跡とディレクトリスキャンによるフォールバックは通常どおり処理されます。
