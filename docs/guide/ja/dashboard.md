...
各 SSE ティックでダッシュボードは `/api/events/{id}` から最新イベントを読み取り、`notify` ヒント（`run-end` 成功、`guard-fail`、または `escalation`）が含まれている場合、ネイティブ OS 通知を発行します。ディスパッチャーは環境に応じた適切なチャンネルを選択します：macOS ネイティブアプリは `nativeNotify` WebKit ブリッジを使用し、Tauri シェルは `tauri-plugin-notification` でバックアップされた `notify` コマンドを呼び出し、通常のブラウザは Web Notification API を使用します（許可を要求した後）。サポートされていない環境や許可されていない環境はサイレントにフォールバックします。通知テキストは `notifications.*` i18n キーでローカライズされます。
