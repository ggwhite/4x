...
- **Built-in denylist** (always applied): `*_TOKEN`, `*_KEY`, `*_SECRET`, `*_PASSWORD`, `*_CREDENTIALS`, `*_SECRETS`, `AWS_*`, `SECRET_*`, `GITHUB_TOKEN`, `GH_TOKEN`, `DATABASE_URL`, `REDIS_URL`, `MONGODB_URI`, `PGPASSWORD`, `MYSQL_PWD`. Add more via `runner_env.denylist`.
- **Built-in credential-keyword substring match** (always applied, in addition to the glob denylist above): any variable whose name **contains** `PASSWORD`, `SECRET`, `TOKEN`, `CREDENTIAL`, `PASSWD`, or `PWD` (case-insensitive) is also filtered — this catches naming conventions the glob list can't (e.g. `PGPASSWORD`, `MYSQL_PWD`). This is intentionally best-effort and can over-match (e.g. `MY_TOKENIZER`); rescue a false positive via `runner_env.allowlist` or per-runner `env_allowlist`.
- **Per-runner built-in allowlist** (keyed by runner **name**): `claude`/`agy` → `ANTHROPIC_API_KEY`/`ANTHROPIC_*`/`CLAUDE_*` plus alternate-provider auth (`AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`/`AWS_SESSION_TOKEN`/`AWS_REGION`/`AWS_PROFILE` for Bedrock, `GOOGLE_APPLICATION_CREDENTIALS`/`GOOGLE_CLOUD_PROJECT`/`CLOUD_ML_REGION`/`ANTHROPIC_VERTEX_*` for Vertex, `AZURE_*` for Azure); `codex` → `OPENAI_API_KEY`/`OPENAI_*`/`CODEX_*`; `gemini` → `GEMINI_API_KEY`/`GOOGLE_API_KEY`/`GOOGLE_*`/`GEMINI_*`; `copilot` → `GITHUB_TOKEN`/`GH_TOKEN`/`COPILOT_*`/`GITHUB_COPILOT_*`; `opencode` → `ANTHROPIC_*`/`OPENAI_*`/`OPENROUTER_*`/`OPENCODE_*` plus `GROQ_API_KEY`/`DEEPSEEK_API_KEY`/`MISTRAL_API_KEY`/`XAI_API_KEY`/`GEMINI_API_KEY`/`GOOGLE_*`/`TOGETHER_API_KEY`; `agent`/`cursor` → `CURSOR_*`/`ANTHROPIC_*`/`OPENAI_*`. The name is the key in `runners`; if it doesn't match a known runner, the command basename (minus `.exe`/`.cmd`/`.bat`) is tried as a fallback, so an absolute-path or wrapper `command` still resolves. Extend per runner via `env_allowlist`.
- **Base allowlist for all runners**: `GITHUB_TOKEN`/`GH_TOKEN` are always allowed regardless of runner (even unknown ones), since any role may shell out to `git`/`gh`.
...
- Patterns are `*` globs matched **case-insensitively**. A malformed glob in `runner_env.denylist` (invalid pattern syntax) fails safe — it is treated as matching everything rather than silently doing nothing — and logs a `slog.Warn` so the typo is observable; rescue any unintended over-filtering via `runner_env.allowlist`.
- Variables that are filtered out are surfaced via `slog.Debug` (key names only, never values).
