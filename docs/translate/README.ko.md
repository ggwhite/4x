...
| **Issue-First MR 플로우** | 선택적 `issue_tracker` 모드: `4x new`가 issue를 생성하거나 연결하고, `4x done`은 로컬 병합 대신 branch를 push하고 PR/MR을 엽니다. 각 저장소의 remote로 GitHub인지 GitLab(셀프 호스팅 포함)인지 자동 감지하며, 추가 설정이 필요 없습니다. |
...
- **대규모 리팩토링을 제어합니다.** 단일 AI 세션으로 처리하기 어려운 대규모 변경 — God Object 분리, 패키지 추출, API 마이그레이션 — 을 의존 관계가 있는 여러 feature로 분할하고 각각에 적절한 profile을 지정할 수 있습니다. 4x가 단계 간 순서, 리뷰, 검증을 처리하므로 단일 컨텍스트 윈도우의 한계를 초과하지 않습니다.
