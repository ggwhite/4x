...
| **Issue-First MR フロー** | オプションの `issue_tracker` モード：`4x new` が issue を作成またはリンクし、`4x done` はローカルマージの代わりに branch を push して PR/MR を開く。各リポジトリの remote から GitHub か GitLab（セルフホストを含む）かを自動判定、追加設定は不要。 |
