...
func runGate(dir string, opts gateOpts) error {
	...
	if opts.pre {
		return runGatePre(ws, dot, resolved)
	}
	return runGatePost(ws, dot, resolved)
}

// runGatePre 讀 candidates.json，對既有 feature 做 Jaccard 去重，把倖存者寫成 gate-input.json。
func runGatePre(ws *protocol.Workspace, dot string, cfg evolution.ResolvedEvolution) error {
...
func runGatePost(ws *protocol.Workspace, dot string, cfg evolution.ResolvedEvolution) error {
