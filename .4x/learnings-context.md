...
- 新增 cobra subcommand 一律一檔一指令，即使 task-brief 寫「一檔含兩個 command」也要遵循 codebase 既有慣例，否則 review 必被打回。
...
- 程式碼改動 protocol types 或新增 test 檔案後，commit 前應執行 make check-docs-sync 並在同一 PR 更新對應 docs/guide/ 文件，避免文件欠債累積。
