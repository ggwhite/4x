...
function renderBatchPanel(status) {
  ...
  if (status && status.queue && status.queue.length) {
    ...
    const done = status.queue.filter(q => q.state === 'done');
    const active = status.queue.filter(q => q.state !== 'done');
    let waitNum = 0;
    const renderItem = q => {
      let icon, cls;
    ...
    queueHtml = active.map(renderItem).join('') + (done.length ? `<div class="batch-q-divider"></div>` + done.map(renderItem).join('') : '');
  }
