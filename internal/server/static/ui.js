...
function showConfirm(title, message, onConfirm) {
  ...
  modal.innerHTML = `<div class="modal-panel fade-in" style="width:380px"><div class="p-5"><div class="font-bold text-sm mb-2">${title}</div><div class="text-sm text-zinc-400">${message}</div></div><div class="flex justify-end gap-2 px-5 pb-4"><button id="confirm-cancel" class="px-4 py-1.5 text-xs rounded border transition-colors" style="border-color:var(--border);color:var(--text-3)" onmouseenter="this.style.background='var(--hover)'" onmouseleave="this.style.background='transparent'">${t('common.cancel')}</button><button id="confirm-ok" class="px-4 py-1.5 text-xs rounded bg-emerald-600 hover:bg-emerald-500 text-white font-semibold transition-colors">${t('common.confirm')}</button></div></div>`;
...
${recent.length>0?`<div class="dash-card"><div class="text-[10px] font-bold dash-muted uppercase tracking-wider mb-4">${t('dashboard.recentCompletions')}</div><div class="space-y-2">${recent.map(f=>`<div class="flex items-center gap-3 py-1.5 cursor-pointer rounded px-2 -mx-2 transition-colors" style="hover:background:var(--bg-hover)" onmouseenter="this.style.background='var(--bg-hover)'" onmouseleave="this.style.background=''" onclick="current='${f.id}';load();loadDetail(${JSON.stringify(f).replace(/"/g,'&quot;')})"><span class="text-emerald-500/60 text-xs">✓</span><span class="text-xs font-semibold text-emerald-400/80">${f.id}</span><span class="text-xs dash-sub truncate flex-1">${esc(f.name)}</span>${f.round?`<span class="text-[10px] dash-muted">${f.round}R</span>`:''}</div>`).join('')}</div></div>`:''}
