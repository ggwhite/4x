...
// 統計 Completed（protocol.BatchCompleted 為真）/ Failed（blocked、needs-attention）/ Remaining（Total 扣除前兩者）。
// outcome 由呼叫端依結束原因傳入（completed / stopped / interrupted / crashed），
// runningFeature 與 panicMsg 僅 interrupted / crashed 時有意義，其餘傳空字串即可。
func BuildBatchReport(ws *protocol.Workspace, plan *BatchPlan, statusMap map[string]protocol.Status,
	...
	for _, entry := range plan.Schedule {
		...
		report.Total++
		switch {
		case protocol.BatchCompleted(status):
			report.Completed++
		case status == protocol.StatusBlocked || status == protocol.StatusNeedsAttention:
			report.Failed++
		}
	}
